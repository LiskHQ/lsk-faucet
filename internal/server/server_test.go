...
func (m *MockTxBuilder) GetContractInstance() *bindings.Token {
	args := m.Called()
	return args.Get(0).(*bindings.Token)
}

func (m *MockTxBuilder) TransferETH(ctx context.Context, to string, value *big.Int) (common.Hash, error) {
	args := m.Called(ctx, to, value)
	return args.Get(0).(common.Hash), args.Error(1)
}

func (m *MockTxBuilder) TransferERC20(ctx context.Context, to string, value *big.Int, balance *big.Int) (common.Hash, error) {
	args := m.Called(ctx, to, value, balance)
	return args.Get(0).(common.Hash), args.Error(1)
}
...
func TestHandleClaim(t *testing.T) {
	...
	expectedAmount := chain.TokenToWei(1.0, 18)
